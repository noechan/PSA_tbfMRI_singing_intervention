function TP12_Sing_alongvsBaseline_TU(subjects, outdir, varargin)
%PostvsPre_Sing_alongvsBaseline_TU
%----------------------------------------------------------------------------------------
...
for sbj = 1:size(subjects, 1)
    ...
    con_idx_pre=find(ismember({SPM.xCon.name}, {'Sing_along>baseline'}));
    ...
    cd(sub_path_post)
    load('SPM.mat')
    con_idx_post=find(ismember({SPM.xCon.name}, {'Sing_along>baseline'}));
    ...
    if ~isempty(con_idx_pre) && ~isempty(con_idx_post)
        ...
        matlabbatch{n}.spm.util.imcalc.output =['Sing_along>baseline_TP2>TP1_' subjects(sbj).name];
