...
panames(ismember({panames.name},{'.','..','PPA','ID158'}))=[]; %ID158 dropout
    %SINGING MEMORY PERFORMANCE: sub1 (ID135) wrong recordings, sub2(ID136) some lis, base, singa &
...
for sub=1:numel(panames)
    ...
    cd (sub_path1)
    if sub~= 11 %all baseline incorrect
    load(['baseline_true_index_Tydyy_' (fnames(sub,:)) '.mat'])
    end
    if sub~=10 && sub~=11 && sub~=13 %all listen incorrect
  ...
  elseif sub==10  || sub==11  || sub==13 %all listen incorrect
  ...
  elseif sub==11 %all baseline incorrect
      missed_duration_base=zeros(1,20);
      s.missed_onsets_base=onsets{1,4};
  end
  
  if sub~=9 %all trials correct
      if ~isempty(s)
          ss=struct2cell(s);
          for iii=1:size(ss,1)
              ss{iii,1}=ss{iii,1}';
