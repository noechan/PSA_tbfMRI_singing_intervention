...
panames(ismember({panames.name},{'.','..','ID109','ID123','ID128'}))=[];
    %SINGING PERFORMANCE SUMMARY: sub1(ID102) some base false; sub2(ID104) all trials correct;
    ...sub3(ID106) some singa false, sub4(ID110) some lis, singa && singm false, all
    ...lis false, sub5(ID112) some singa false, sub6(ID113) all trials correct,
    ...sub7(ID114) some lis, singa & singm false, sub8(ID116) some singa false,
    ...sub9(ID121) some lis & singa false,
    ...sub10(ID122) some lis, singa & singm false, sub11(ID124) some lis & singm false,
    ...sub12(ID127) some base, lis, singa & singm false, sub13(ID134) some 
    ...lis, base & singm false
for sub=1:numel(panames)
  ...
  if sub~=2 && sub~=6 %sub02 and sub06 all trials correct
      if ~isempty(s)
          ss=struct2cell(s);
          for iii=1:size(ss,1)
              ss{iii,1}=ss{iii,1}';
